...
isMonoChain ly with (ly)
  _ | (MkLayout m Z [n]) = [%search]
  _ | (MkLayout m (S l) (n1 :: n2 :: ns')) = (%search) :: isMonoChain (MkLayout n1 l (n2 :: ns'))


-- Proof that every type in ParaChain is a Show
isShowChain : (ly : Layout mIn ns) -> HVect (map Show (ParaChain ly))
isShowChain ly with (ly)
  _ | (MkLayout m Z [n]) = [%search]
  _ | (MkLayout m (S l) (n1 :: n2 :: ns')) = (%search) :: isShowChain (MkLayout n1 l (n2 :: ns'))


-- Proof that every type in ParaChain is a VSpace
isVSpaceChain : (ly : Layout mIn ns) -> HVect (map VSpace (ParaChain ly))
isVSpaceChain ly with (ly)
  _ | (MkLayout m Z [n]) = [%search]
  _ | (MkLayout m (S l) (n1 :: n2 :: ns')) = (%search) :: isVSpaceChain (MkLayout n1 l (n2 :: ns'))
...
export
showParas : (ly : Layout mIn layers) -> HVect (ParaChain ly) -> String
showParas (MkLayout mIn Z [n]) v = show v
showParas (MkLayout mIn (S l) (n1 :: n2 :: ns)) (p :: ps) = show p ++ ", " ++ 
  showParas (MkLayout n1 l (n2 :: ns)) ps

public export
{ly : Layout mIn layers} -> Show (HVect (ParaChain ly)) where
  show {ly} ps = showParas ly ps
...
makeMLP : (ly : Layout mIn layers) -> 
    PLens (HVect (ParaChain ly)) (V mIn) (V (last layers))
makeMLP (MkLayout mIn Z [nOut]) = MkPLens fwd' bwd'
  where
    ...
    fwd' : (HVect (ParaChain Ly), V mIn) -> V (nOut)
    fwd' ([p], v) = lr.fwd (p, v)
    bwd' : (HVect (ParaChain Ly), V mIn, V nOut) -> (HVect (ParaChain Ly), V mIn)
...
makeMLP (MkLayout mIn (S l) (n1 :: n2 :: ns)) =  MkPLens fwd' bwd'
  where
    ...
    mlp' : PLens (HVect (ParaChain Ly)) (V (inN Ly)) (V (outN Ly))
    mlp' = makeMLP Ly --<< recurse
    -- compose with the bottom layer
    mlpComp : PLens (ParaBlock mIn n1, HVect (ParaChain Ly))
                    (V mIn)
                    (V (last (n2 :: ns)))
    ...
    fwd' : (HVect (ParaChain Ly'), V mIn) -> V (last (n1 :: n2 :: ns))
    fwd' (p1 :: ps, vm) = mlpComp.fwd ((p1, ps), vm)
    bwd' : (HVect (ParaChain Ly'), V mIn, V (last (n1 :: n2 :: ns))) ->
           (HVect (ParaChain Ly'), V mIn)
    bwd' (pmn1 :: pmns, s, a) =
...
initParaChain : (ly : Layout mIn layers) ->
    Stream Double -> (HVect (ParaChain ly), Stream Double)
